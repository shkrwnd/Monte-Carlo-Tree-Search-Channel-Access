...
public:
	...
	State next_state(void) {
		...
		State next = State(this->value + next_move * this->discount , t, new_depth,this->sum+ next_move);
		return next;
	}
	bool terminal(void) {
		if (this->depth <= 0) {
...
ostream& operator<< (ostream& os, State& n) {
	os << "Value: " << n.value << "; " <<"Sum: " << n.sum << " moves: " << n.put_moves() << " turn: " << n.depth << endl;
	return os;
}
